...
__global__ void kernel_160_winograd_AtIA(float *pInputs, float *pBiases, float *pScales, float *pOutputs) {
	...
	switch(Iny) {
		...
		case 2:
			...
			pOutputs[(((Tilex<<2)+1+Inx)*16 + (Tiley<<2)+3)*160 + kz] = o;
			break;
		case 3:
			if (Tiley == 3) break;
			x = Inx*6;
			...
			pOutputs[(((Tilex<<2)+1+Inx)*16 + (Tiley<<2)+4)*160 + kz] = o;
			break;
	}
}
